...
# 5) combine features of solution 3)and 4)
class Solution:
    def robotSim(self, commands, obstacles):
        """
        :type commands: List[int]
        ...
        for command in commands:
            if command == -1:
                ...
                if (direction == 0 or direction == 2) and x not in dict_x: y += dy[direction] * command
                elif (direction == 1 or direction == 3) and y not in dict_y: x += dx[direction] * command
                else:
                    while command:
                        if (direction == 0 or direction == 2) and y+dy[direction] not in dict_x[x]: y = y + dy[direction]
                        elif (direction == 1 or direction == 3) and x+dx[direction] not in dict_y[y]: x = x + dx[direction]
                        command -= 1
                if x**2 + y**2 > max_dist: max_dist = x**2 + y**2
        return max_dist
